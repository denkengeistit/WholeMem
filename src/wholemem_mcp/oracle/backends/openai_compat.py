...
class OpenAICompatBackend(OracleBackend):
    ...
    def __init__(
        ...
        self._api_key = api_key or os.environ.get("WHOLEMEM_LLM_API_KEY", "")
    ...
    async def generate(self, messages: list[dict], max_tokens: int = 2048) -> str:
        ...
        try:
            # base_url may already include /v1 (e.g. from LLMConfig)
            url = self._base_url
    ...
    async def health_check(self) -> bool:
        ...
        try:
            models_url = f"{self._base_url}/models" if self._base_url.endswith("/v1") else f"{self._base_url}/v1/models"
